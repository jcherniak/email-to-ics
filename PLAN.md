...
- Active work: migrate the newer self-hosted Chrome extension from `node-server-chrome-shared` onto `main`, while preserving `main` as the PHP/prompt source of truth.
  ...
  - [ ] Re-run PHP PHPUnit parity tests in a PHP 8.2+ environment.
...
- PHP parity tests are still blocked locally because this worktree has PHP 7.4.33 and no `vendor/bin/phpunit`; `composer.lock` requires PHP 8.2+ for PHPUnit 11 and other dependencies.
