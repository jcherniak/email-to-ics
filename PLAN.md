...
- Completed follow-up: made command-line/process environment precedence over `.env` general across the app. `Dotenv::createImmutable()` protects `getenv()`, but app code reads `$_ENV`; added `Jcherniak\EmailToIcs\Config\Environment` to load dotenv and then overlay process environment values into `$_ENV`/`$_SERVER`. `index.php` now uses the shared loader, and `EnvironmentPrecedenceTest` covers process values winning over `.env` in `getenv()`, `$_ENV`, `$_SERVER`, and `Environment::get()`. Verified with PHP lint for touched files, targeted environment/geocoder tests, and full `vendor/bin/phpunit` (51 tests, 159 assertions, 1 expected Google skip).
- Completed follow-up: added live geocoder integration tests. Tests probe real provider keys from process environment first, then local `.env`; missing/invalid/unauthorized providers log JSON error details and skip by default. `TESTS_REQUIRE_GEOCODER_INTEGRATION=mapbox`, `google`, provider lists, or `all`/`1` makes selected provider setup failures hard failures. Verified Mapbox-required mode with `TESTS_REQUIRE_GEOCODER_INTEGRATION=mapbox vendor/bin/phpunit --filter GeocoderIntegrationTest`; Mapbox passed and Google skipped because Google currently returns `REQUEST_DENIED` billing-required. Full `vendor/bin/phpunit` passed with 50 tests, 155 assertions, 1 expected Google skip.
- Completed follow-up: made location enrichment pluggable with `GeocoderInterface`, `ChainGeocoder`, `MapboxGeocoder`, and `GooglePlacesGeocoder`. Default order is `GEOCODER_ORDER=mapbox,google` in the existing tracked `.env.template`; no `.env.example` was created and local `.env` secrets were not committed. If every configured geocoder fails, the original event location is preserved, the ICS path continues, and one diagnostic error email includes each provider's failure details. Verified with PHP lint for backend/test/geocoder files, `vendor/bin/phpunit` (47 tests, 148 assertions), and a live Mapbox lookup for Presidio Theatre using the local `.env` key.
- Completed follow-up: latest Apache logs show Google Maps Places Text Search returns `REQUEST_DENIED` because billing is not enabled on the Google Cloud project. Maps lookup is now optional enrichment: failed Maps lookups preserve the AI/original event location, continue sending the ICS, and send a separate diagnostic error email with Maps status/error details. Error emails now send directly through the mailer without logging a missing ICS attachment as an upstream problem. Added regression coverage for the diagnostic email and preserved original location. Verified with `php -l index.php`, `php -l tests/DependencyInjectionRefactorTest.php`, and `vendor/bin/phpunit` (46 tests, 142 assertions).
- Completed follow-up: updated backend/env/Chrome extension model IDs from `provider/model-latest` to OpenRouter's tilde-prefixed `~provider/model-latest` aliases, keeping the same preferred order. URL detection uses `~google/gemini-flash-latest` first and `~openai/gpt-mini-latest` as fallback. Rebuilt the extension bundle and verified with PHP lint, JS syntax checks, `vendor/bin/phpunit --filter UrlDetectionTest`, and `vendor/bin/phpunit` (45 tests, 134 assertions).
- Completed follow-up: latest Apache logs showed the Presidio request now succeeds and sends three emails, but Google Maps lookup for `Presidio Theatre, 99 Moraga Avenue, San Francisco, CA 94129` returned an empty result. Added backend logging around the Maps text-search request, HTTP status, curl errors, response body snippet, parsed API status/error message, result count, and first result metadata. Verified with `php -l index.php` and `vendor/bin/phpunit` (45 tests, 134 assertions).
