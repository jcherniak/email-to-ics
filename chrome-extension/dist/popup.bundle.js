...
(() => {
  ...
  var BrowserIcsGenerator = class {
    generateIcs(events, config = {}) {
      ...
      events.forEach((event, index) => {
        ...
        const end2 = event.dtend || this.defaultEnd(event.dtstart, event.isAllDay);
        lines.push(`DTEND${this.datePrefix(event)}:${this.formatDate(end2, event)}`);
    ...
    async restoreState() {
      ...
      try {
        ...
        if (state && Date.now() - state.timestamp < 36e5) {
          ...
          const tentativeToggle = document.getElementById("tentative-toggle");
          const multidayToggle = document.getElementById("multiday-toggle");
          const screenshotToggle2 = document.getElementById("screenshot-toggle");
          ...
          if (screenshotToggle2 && typeof form.screenshot === "boolean")
            screenshotToggle2.checked = form.screenshot;
  ...
  document.addEventListener("DOMContentLoaded", async function() {
    ...
    if (buildEl)
      buildEl.textContent = `Build: ${"05/01/2026, 15:46:36"}`;
    ...
    function disableForm(disable = true) {
      ...
      if (modelSelect)
        (0, import_jquery.default)(modelSelect).prop("disabled", disable);
        ...
        return new Promise((resolve) => {
          chrome.runtime.sendMessage({ action: "captureScreenshot" }, (response) => {
            ...
            if (response && response.success && response.screenshot) {
              ...
              compressImage(original, 0.25, 0.6).then((compressed) => {
                try {
    ...
    function initModelSelect2() {
      ...
      if (select2Initialized) {
        (0, import_jquery.default)(modelSelect).select2("destroy");
        select2Initialized = false;
      }
      (0, import_jquery.default)(modelSelect).select2({
        placeholder: (0, import_jquery.default)(modelSelect).data("placeholder") || "Search or select a model",
        width: "100%"
      });
      (0, import_jquery.default)(modelSelect).off("change.popup");
      (0, import_jquery.default)(modelSelect).on("change.popup", () => {
    ...
    function syncModelSelectValue() {
      ...
      (0, import_jquery.default)(modelSelect).val(value).trigger("change");
    }
    function getOfflinePriorityModels() {
    ...
    function buildModelOptions(allModels) {
      ...
      const priorityList = PRIORITY_MODEL_IDS.map((id) => {
        ...
        if (model && !seen.has(model.id)) {
          seen.add(model.id);
          return { ...model, isPriority: true };
        }
        return null;
      }).filter(Boolean);
      const otherModels = normalized.filter((model) => !seen.has(model.id)).sort((a, b) => a.name.localeCompare(b.name)).map((model) => ({ ...model, isPriority: false }));
      return [...priorityList, ...otherModels];
    }
    function resolveInitialModel(preferredId, options) {
      const selectable = options;
      const candidates = [preferredId, DEFAULT_MODEL_ID].filter(Boolean);
    ...
    async function loadModels(forceRefresh = false) {
      ...
      if (modelSelect)
        (0, import_jquery.default)(modelSelect).prop("disabled", true);
      try {
        ...
        if (modelSelect)
          (0, import_jquery.default)(modelSelect).prop("disabled", false);
      }
    }
    ...
    async function loadSettingsIntoUI() {
      ...
      if (preextractToggle)
        preextractToggle.checked = settings.preextract !== false;
    ...
    async function populateDefaultModels(selectedModel2) {
      try {
        ...
        } else if (selectedModel2 && !models.some((m) => !m.isSeparator && m.id === selectedModel2)) {
    ...
    async function generateICS() {
      ...
      let multiday = multidayToggle.checked;
      ...
      try {
        ...
        try {
          ...
          console.log("\u{1F4C4} getPageContent response:", contentResponse);
        } catch (msgErr) {
        ...
        if (!multiday && shouldExtractEqualPerformances(`${pageContent.url}
${pageContent.html}
${pageContent.text || ""}`, instructions)) {
          multiday = true;
          console.log("\u{1F4C5} Multi-day mode enabled automatically for equal performance schedule");
        }
        requestData.textContent = JSON.stringify({
          ...
          multiday,
          reviewFirst,
          autoMultiday: multiday && !multidayToggle.checked
        }, null, 2);
        let screenshot = null;
        if (screenshotToggle2?.checked !== false) {
          ...
          console.log("\u{1F4F8} Screenshot disabled by user");
        }
        ...
        if (reviewFirst) {
          ...
          const eventDetails = events.length === 1 ? formatEventDateSummary(events[0]) : `${formatEventDateSummary(events[0])} - ${formatEventDateSummary(events[events.length - 1])}`;
    ...
    async function loadSharedPromptPolicy() {
      const response = await fetch(chrome.runtime.getURL("system_prompt_policy.xml"));
      if (!response.ok) {
        throw new Error(`Failed to load shared prompt policy: HTTP ${response.status}`);
      }
      const policy = (await response.text()).trim();
      if (!policy) {
        throw new Error("Shared prompt policy is empty");
      }
      return policy;
    }
    function currentDateForPrompt() {
    ...
    function phpEventSchema() {
      return {
        ...
        properties: {
          ...
          description: { type: "string", description: "Plain text description using \\n for newlines." },
          htmlDescription: { type: "string", description: "HTML description using basic tags only." },
          dtstart: { type: "string", description: "Start date/time as ISO 8601: YYYY-MM-DDTHH:MM:SS, or YYYY-MM-DD for all-day events." },
          dtend: { type: "string", description: "End date/time as ISO 8601: YYYY-MM-DDTHH:MM:SS, or YYYY-MM-DD for all-day events." },
          timezone: { type: "string", description: "Valid IANA/PHP timezone identifier." },
          location: { type: "string", description: "Venue name or address." },
          url: { type: "string", description: "Original source URL or event-specific URL." },
    ...
    async function callAIModel(pageData, instructions, model, tentative, multiday, screenshot) {
      ...
      const today = currentDateForPrompt();
      const sharedPromptPolicy = await loadSharedPromptPolicy();
      const eventDataSchema = multiday ? { type: "array", items: phpEventSchema(), minItems: 1, maxItems: 10 } : phpEventSchema();
...
${multiday ? `## Multi-Day Mode (ENABLED):
...
ALWAYS return eventData as an ARRAY of event objects, even if there is only one event.` : `## Single Event Mode (DEFAULT):
...
# DATE PARSING RULES
The current year is ${today.year} and today (${today.mmdd}) is ${today.iso}.
When parsing dates:
...
# TIMEZONE INFERENCE
If a location is specified, infer the appropriate timezone immediately.
...
# FIELD INSTRUCTIONS
- Return PHP-compatible eventData using dtstart and dtend, not Chrome-only start_date/start_time fields.
...
# OUTPUT FORMAT
Return a JSON object containing the structured event data. DO NOT return an ICS string.
For single-event mode, eventData must be one event object.
For multi-day mode, eventData must be an array of event objects.
...
# ERROR HANDLING
IF NO DATES ARE FOUND ANYWHERE IN THE CONTENT, set success to false with errorMessage "Content didn't contain dates or times" and return eventData as ${multiday ? "[]" : "an empty object with required fields if schema compliance requires it"}.

# SCREENSHOT GUIDANCE (IF PROVIDED)
The browser extension may provide one screenshot image captured from the current page. Use it as visual evidence to identify the main event and confirm dates, times, venue, or selected/visible performance. Do not infer facts from the screenshot that conflict with explicit HTML or user instructions unless the screenshot clearly shows the user's selected/primary context.
...
You MUST include this URL in the 'url' field of the event data, and also add it to the bottom of both the plain text and HTML descriptions.`;
      const eventSchema = {
        ...
        properties: {
          ...
          errorMessage: { type: "string", description: "Error message if success is false, empty string otherwise." },
...
${cleanedHtml}`;
    ...
    function parseAiResponse(response) {
      try {
        ...
        const events = Array.isArray(parsed.eventData) ? parsed.eventData : parsed.eventData ? [parsed.eventData] : Array.isArray(parsed.events) ? parsed.events : Array.isArray(parsed) ? parsed : null;
    ...
    function eventStartParts(eventData) {
      ...
      return {
        ...
        time: eventData.start_time || null
      };
    }
    function formatEventDateSummary(eventData) {
      const start2 = eventStartParts(eventData);
      return `${start2.date}${start2.time ? " at " + start2.time : " (All day)"}`;
    }
    function instructionsFocusSpecificDate(instructions) {
      return /\b(only|focus|specific|particular|just|choose|use)\b.{0,80}\b(date|performance|session|show|event|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|\d{1,2}[/-]\d{1,2})\b/i.test(instructions || "");
    }
    function extractDateTimeCandidates(content) {
      const normalized = content.replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ");
      const candidates = [];
      const patterns = [
        /\b(?:Mon|Tue|Tues|Wed|Thu|Thurs|Fri|Sat|Sun)(?:day)?[,]?\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{1,2},?\s+\d{4}\s+(?:at\s+)?\d{1,2}(?::\d{2})?\s*(?:AM|PM|A\.M\.|P\.M\.)\b/gi,
        /\b(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{1,2},?\s+\d{4}\s+(?:at\s+)?\d{1,2}(?::\d{2})?\s*(?:AM|PM|A\.M\.|P\.M\.)\b/gi
      ];
      for (const pattern of patterns) {
        for (const match of normalized.matchAll(pattern)) {
          candidates.push(match[0].toLowerCase().replace(/\bat\s+/i, "").replace(/\s+/g, " ").trim());
        }
      }
      return Array.from(new Set(candidates));
    }
    function shouldExtractEqualPerformances(content, instructions) {
      if (instructionsFocusSpecificDate(instructions))
        return false;
      if (!/\b(ticket|tickets|buy tickets|performance|performances|session|sessions|showtime|showtimes|concert|opera|theatre|theater|screening|show)\b/i.test(content)) {
        return false;
      }
      return extractDateTimeCandidates(content).length >= 2;
    }
    async function generateICSContent(events, tentative) {
      ...
      const eventDataArray = events.map((eventData) => {
        const start2 = eventStartParts(eventData);
        const endRaw = eventData.dtend || eventData.end_date;
        const end2 = endRaw && eventData.end_time ? `${endRaw}T${eventData.end_time}:00` : endRaw;
        return {
          summary: eventData.summary,
          description: sanitizeDescription(eventData.description || ""),
          htmlDescription: eventData.htmlDescription || "",
          location: eventData.location || "",
          dtstart: eventData.dtstart || `${start2.date}${start2.time ? `T${start2.time}:00` : ""}`,
          dtend: end2,
          timezone: eventData.timezone || fallbackTz,
          isAllDay: eventData.isAllDay ?? !start2.time,
          status: tentative ? "tentative" : "confirmed",
          url: eventData.url
        };
      });
      return icsGenerator.generateIcs(eventDataArray, {});
    }
    ...
    saveSettingsButton?.addEventListener("click", async () => {
      ...
      const preextractSetting = document.getElementById("preextract-toggle")?.checked ?? true;
...
${event.data.data.selectedText}`;
    ...
    async function sendEmail(events, icsContent, tentative, emailSubject) {
      ...
      try {
        ...
        console.log(`\u{1F4E8} Sending ${events.length} email(s)...`);
        const results = [];
...
` : ""}${cleanDesc ? `Description: ${cleanDesc}` : ""}`;
          ...
          console.log(`\u{1F4E5} Email ${i + 1}/${events.length} send response:`, response);
          if (response.success) {
        ...
        const successMessage2 = events.length === 1 ? "Calendar invite sent successfully!" : `All ${events.length} calendar invites sent successfully!`;
        console.log(`\u2705 ${successMessage2}`);
        return { message: successMessage2 };
      } catch (error) {
