(() => {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const formSection2 = document.getElementById("form-section");
    ...
    const reviewSection2 = document.getElementById("review-section");
    const reviewContent2 = document.getElementById("review-content");
    const reviewRecipient2 = document.getElementById("review-recipient");
    const reviewSubject2 = document.getElementById("review-subject");
    const sendButton = document.getElementById("send-button");
    const rejectButton = document.getElementById("reject-button");
    const processingView2 = document.getElementById("processingView");
    ...
    function disableReviewButtons2(disable = true) {
...
`;
      ...
      hideReviewStatus2();
      formSection2.style.display = "none";
      reviewSection2.style.display = "none";
      processingView2.style.display = "block";
    ...
    async function sendReviewedICS() {
      if (!reviewData2 || !reviewData2.confirmationToken) {
        showReviewStatus("Error: Missing confirmation data.", "error");
        return;
      }
      showReviewStatus("Sending confirmation...", "loading");
      disableReviewButtons2();
      try {
        const formData = new URLSearchParams();
        formData.append("confirmationToken", reviewData2.confirmationToken);
    ...
    backToFormButton?.addEventListener("click", () => {
      processingView2.style.display = "none";
      formSection2.style.display = "block";
      disableForm(false);
    });
    document.addEventListener("keydown", function(event) {
      ...
      if (event.key === "Enter" && (event.ctrlKey || event.metaKey) && !isTypingArea) {
        if (formSection2.style.display !== "none" && processingView2.style.display === "none") {
          event.preventDefault();
          convertButton?.click();
        }
      }
    });
    checkAuthenticationAndFetchConfig2().then(() => {
    ...
    function showReviewSection(data) {
      ...
      console.log("reviewData set:", reviewData2);
      reviewRecipient2.textContent = data.recipientEmail || "Unknown";
      reviewSubject2.textContent = data.emailSubject || "No Subject";
      console.log("Review recipient/subject populated");
      console.log("About to parse ICS content using ical.js");
      reviewContent2.innerHTML = parseAndDisplayIcs(data.icsContent || "");
      console.log("ICS content parsed and set to innerHTML");
      formSection2.style.display = "none";
      processingView2.style.display = "none";
      reviewSection2.style.display = "block";
