...
import { BrowserIcsGenerator, EventData } from './browser-ics';
import 'bootstrap';
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Initialize browser ICS generation.
    ...
    async function generateICS() {
        ...
        try {
            ...
            if (!multiday && shouldExtractEqualPerformances(`${pageContent.url}\n${pageContent.html}\n${pageContent.text || ''}`, instructions)) {
                multiday = true;
                console.log('📅 Multi-day mode enabled automatically for equal performance schedule');
            }
    ...
    function currentDateForPrompt(): { year: number; mmdd: string; iso: string } {
        const now = new Date();
        return {
...
ALWAYS return eventData as an ARRAY of event objects, even if there is only one event.`
...
${customPrompt}`;
    ...
    function instructionsFocusSpecificDate(instructions: string): boolean {
    ...
    function extractDateTimeCandidates(content: string): string[] {
    ...
    function shouldExtractEqualPerformances(content: string, instructions: string): boolean {
        if (instructionsFocusSpecificDate(instructions)) return false;
    ...
    // ICS generation using browser-local PHP-compatible event data.
    async function generateICSContent(events: any[], tentative: boolean): Promise<string> {
        ...
        const eventDataArray: EventData[] = events.map(eventData => {
            ...
            const end = endRaw && eventData.end_time ? `${endRaw}T${eventData.end_time}:00` : endRaw;
            return {
                summary: eventData.summary,
                description: sanitizeDescription(eventData.description || ''),
                htmlDescription: eventData.htmlDescription || '',
                location: eventData.location || '',
                dtstart: eventData.dtstart || `${start.date}${start.time ? `T${start.time}:00` : ''}`,
                dtend: end,
                timezone: eventData.timezone || fallbackTz,
                isAllDay: eventData.isAllDay ?? !start.time,
