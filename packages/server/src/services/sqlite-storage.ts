...
export class SqliteStorageAdapter implements StorageAdapter {
  ...
  async getStats(): Promise<{ total: number; expired: number; size: number }> {
    ...
    try {
      const totalRow = await this.dbGet('SELECT COUNT(*) as count FROM cache') as { count: number };
      const expiredRow = await this.dbGet('SELECT COUNT(*) as count FROM cache WHERE expires_at <= ?', now) as { count: number };
      
      // Get database size (simplified)
      const sizeRow = await this.dbGet('SELECT page_count * page_size as size FROM pragma_page_count(), pragma_page_size()') as { size: number } | undefined;
