...
export class BrowserIcsGenerator {
   ...
   */
  private generateEventId(event: EventData, index: number): string {
   ...
   */
  private formatDateTime(date: Date, timezone?: string, isAllDay?: boolean): string {
    if (isAllDay) {
   ...
   */
  private getDateTimePrefix(timezone?: string, isAllDay?: boolean): string {
    if (isAllDay) {
      return ';VALUE=DATE';
    }
   ...
   */
  private parseEvent(eventContent: string): EventData | null {
    ...
    lines.forEach(line => {
      ...
      switch (property.split(';')[0]) {
        ...
        case 'URL':
          ...
          break;
        case 'STATUS':
          event.status = value.toLowerCase() as 'confirmed' | 'tentative';
          break;
      }
    });
   ...
   */
  private parseIcsDateTime(value: string): string {
