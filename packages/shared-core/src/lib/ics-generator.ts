...
export class IcsGenerator {
   ...
   */
  async generateIcs(
    eventData: EventData | MultiDayEvents, 
    ...
    try {
      ...
      if (config.method) {
        calendar.method(config.method as any);
      }
   ...
   */
  private async addEventToCalendar(
    calendar: any,
    eventData: EventData, 
    fromEmail: string,
    config: Partial<IcsConfig>
    ...
    if (eventData.dtend) {
      ...
      if (!isAllDay) {
        ...
        if (summary.includes('opera')) durationHours = 3;
   ...
   */
  private async generateUID(eventData: EventData): Promise<string> {
   ...
   */
  validateIcs(icsString: string): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
